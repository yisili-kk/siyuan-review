...
export function buildQuestionPrompt(item: ReviewItem, content: string): string {
  ...
  const templateQuestions = item.templateQuestions.map((question, index) => `${index + 1}. ${question}`).join("\n");
  return [
    "你是一个知识管理回顾助手。",
